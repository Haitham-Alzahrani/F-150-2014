# THE FULL RECORD — every finding, in the order it was made

**This is the history of the investigation, moved out of `CLAUDE.md` on
2026-09-18 so that the instruction file stays readable.**

**NOTHING HERE WAS DELETED.** Every section is exactly as it was written,
including the ones later withdrawn — a withdrawn claim is kept with its
withdrawal so the reasoning that produced the error stays visible.

**`CLAUDE.md` now carries the operating file: the truck, the current symptom,
what to capture, the method rules, and how to run everything.** It points here
for the evidence behind any of it. **Read this when you need to know WHY
something is believed, or to check whether a question has already been asked.**

**The state of the whole truck is in
[`DIAGNOSTIC-DASHBOARD.md`](DIAGNOSTIC-DASHBOARD.md), not here.**
